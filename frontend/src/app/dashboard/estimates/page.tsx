...
const ESTIMATE_STATUS_COLOR: Record<string, "default" | "warning" | "info" | "success"> = {
  draft:   "warning",
  issued:  "info",
  ordered: "success",
};
...
function EstimateListPageInner() {
  ...
  const applySearch = () => fetchEstimates(selectedShop);
  ...
  const handleAction = (action: string, id: number) => {
    closeMenu(id);
    switch (action) {
      case "detail":    router.push(`/dashboard/estimates/${id}`);                      break;
      case "edit":      router.push(`/dashboard/estimates/${id}/edit?_r=${Date.now()}`); break;
      case "duplicate": router.push(`/dashboard/estimates/new?copy_from=${id}`);        break;
      case "order":     router.push(`/dashboard/orders/new?from_estimate=${id}`);       break;
      case "delete": {
        const t = estimates.find(e => e.id === id);
  ...
  return (
    <>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Typography variant="h5" fontWeight="bold">見積一覧</Typography>
        ...
        <Stack direction="row" spacing={2} alignItems="center" mb={2}>
          ...
          <FormControl size="small" sx={{ minWidth: 140 }}>
            ...
            >
              ...
              <MenuItem value="issued">提出済み</MenuItem>
              <MenuItem value="ordered">受注済み</MenuItem>
            </Select>
          </FormControl>
        </Stack>
          ...
          <TableBody>
            ...
            {estimates.map(est => {
              ...
              return (
                ...
                >
                  <TableCell>{fmtDate(est.estimate_date ?? est.created_at)}</TableCell>
                  <TableCell>
                    <Chip
                      size="small"
                      label={ESTIMATE_STATUS_LABEL[est.status] ?? est.status}
                      color={ESTIMATE_STATUS_COLOR[est.status] ?? "default"}
                    />
                  </TableCell>
                  <TableCell>{est.party?.name || "-"}</TableCell>
                  <TableCell>{productName}</TableCell>
                  <TableCell>{est.created_by?.display_name || "-"}</TableCell>
                  <TableCell align="right">{fmt(est.grand_total)}</TableCell>

                  <TableCell align="center" onClick={e => e.stopPropagation()}>
                    <IconButton size="small" onClick={e => openMenu(e, est.id)}>
                      <MoreVertIcon fontSize="small" />
                    </IconButton>
                    <Menu
                      anchorEl={menuAnchor[est.id]}
                      open={Boolean(menuAnchor[est.id])}
                      onClose={() => closeMenu(est.id)}
                    >
                      ...
                      <MenuItem onClick={() => handleAction("order", est.id)}>
                        ...
                        <ListItemText>受注作成</ListItemText>
                      </MenuItem>
                      <MenuItem onClick={() => handleAction("delete", est.id)} sx={{ color: "error.main" }}>
